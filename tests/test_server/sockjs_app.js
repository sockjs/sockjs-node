...
exports.install = function(opts, server) {
  const sjs_echo = sockjs.createServer(opts);
  ...
  const sjs_close = sockjs.createServer(opts);
  ...
  const sjs_ticker = sockjs.createServer(opts);
  ...
  const broadcast = {};
  const sjs_broadcast = sockjs.createServer(opts);
  sjs_broadcast.on('connection', function(conn) {
    ...
    conn.on('data', function(m) {
      ...
      for (let id in broadcast) {
        broadcast[id].write(m);
      }
    });
  });

  const sjs_amplify = sockjs.createServer(opts);
