...
exports.install = function(opts, server) {
  ...
  const sjs_echo = sockjs.createServer(Object.assign({}, opts, {prefix: '/echo', response_limit: 4096}));
  sjs_echo.on('connection', echoHandler);
  sjs_echo.attach(server);

  const sjs_echo2 = sockjs.createServer(Object.assign({}, opts, {prefix: '/disabled_websocket_echo', websocket: false}));
  sjs_echo2.on('connection', echoHandler);
  sjs_echo2.attach(server);

  const sjs_echo3 = sockjs.createServer(Object.assign({}, opts, {prefix: '/cookie_needed_echo', jsessionid: true}));
